...
//socket -> bind -> listen -> loop -> select -> if socket fd -> accept else recv, send -> close
...
type -> transport (SOCK_STREAM, SOCK_DGRAM)
protocol -> 0
...
int		main(int ac, char **av)
{
	...
	FD_ZERO(&afds); // global active user tracker
...
The idiomatic, clear way is to use:

INADDR_LOOPBACK (for 127.0.0.1)

INADDR_ANY (for 0.0.0.0: all local interfaces)
